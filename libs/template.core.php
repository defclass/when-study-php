...
//	var $ImgDir		= array('images','js','css');	//图片地址目录
	var $ImgDir		= array( );	//图片地址目录
	var $HtmDir		= 'cache_htm/';		//静态存放的目录
	var $HtmID		= '';				//静态文件ID
	var $HtmTime	= '1';				//秒为单位，默认三分钟
	var $AutoImage	= 1;				//自动解析图片目录开关默认值
	var $Hacker		= "<?php defined('zfcms') OR die('You are Hacker!<br>');";
	var $Compile	= array();
	var $Analysis	= array();
	var $Emc		= array();
	/**
	...
	function CompilePHP(
		...
		if ($content){
			...
			if($this->RunType=='Cache' && !is_file($this->CacheDir.'index.htm')){
				$Ease_name   = 'zfcms';
				$Ease_base   = "<title>$Ease_name</title><a href='http://www.rongyitou.com'>$Ease_name</a>";
