...
class DB {
	...
	static function instance(){
		...
		if(!isset( self::$instance)){
			...
			self::$instance->time = self::$instance->microtime_float();
			include(APP_PATH."config.php");
			self::$instance->connect($db_config["hostname"], $db_config["username"], $db_config["password"], $db_config["database"], $db_config["pconnect"]);
			self::$instance->is_log = $db_config["log"];
			if(self::$instance->is_log){
				$handle = fopen($db_config["logfilepath"]."dblog.txt", "a+");
				self::$instance->handle=$handle;
			}
		}
	...
	//插入
	public function insert($table,$dataArray,$showsql='0') {
		$field = "";
		$value = "";
	...
	//更新
	public function update( $table,$dataArray,$condition="",$showsql='0') {
		if( !is_array($dataArray) || count($dataArray)<=0) {
			$this->halt('没有要更新的数据');
			return false;
		}
